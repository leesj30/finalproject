...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!$result) {
            echo "쿼리 실행 중 오류 발생: " . mysqli_error($conn);
            exit();
        }
        ...
        if ($user) {
            if ($currentPassword == $user['userpw']) {
                //$hashedPassword = password_hash($newPassword, PASSWORD_BCRYPT);
                $update_pw = "UPDATE user SET userpw = '$newPassword' WHERE userid = '$userid'";
                
                if ($conn->query($update_pw) === TRUE) {
                ...
                echo "<script><script>alert('현재 비밀번호가 올바르지 않습니다!');</script>";
            }
        } else {
